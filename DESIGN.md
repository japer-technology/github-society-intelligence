# DESIGN — A Federation of Repository-Native Societies

### The architecture for repo-based AI agencies that think, reflect, and speak to each other across repository boundaries without losing insulation, reflection, or honesty
...
This document specifies the complete architecture for repository-native AI societies — from the substrate of a single insulated thought, through a reflective society able to inspect and revise its own structures, up to a federation of such societies that speaks to itself through governed channels, signed contracts, honest bridges, and an append-only economic ledger.

The architecture rests on three load-bearing claims, all of which must hold together:

1. **A branch is the natural physical substrate for an insulated thought.** A single repository can host a Minsky-style society using only the primitives the forge already gives us.
2. **A society becomes intelligent enough to trust only when it can inspect and revise the structures by which it thinks** — frames, K-lines, analogies, censors, self-models, and self-ideals included.
3. **A society becomes safely networked only when the act of crossing its boundary is held to the same standard as the act of changing its own mind.**

The whole design follows from these three claims. The research grounding lives in [`research/THE-SOCIETY-OF-MIND/`](research/THE-SOCIETY-OF-MIND/), [`research/THE-REPO-IS-SOCIETY/`](research/THE-REPO-IS-SOCIETY/), [`research/THE-SOCIETY-OF-REPO/`](research/THE-SOCIETY-OF-REPO/), and [`research/FORGEJO-SOCIETY-IMPLEMENTATION/`](research/FORGEJO-SOCIETY-IMPLEMENTATION/).
...
A federation of repositories under this architecture is not a multi-agent framework. It is not microservices with a thicker proxy. It is not a chatbot federation. It is a society of bounded cognitive organs that *speak to each other under the same constitutional discipline they apply internally*, with bridges where their vocabularies differ and ledgers where their economies meet.

The maturity ladder this design targets:

```text
Level 3: "Many small agents under one constitution can settle disagreements
          on a branch and merge an action."
...
This design specifies Levels 3 and 4 as foundational, Level 5 as the **minimum networked target**, and Level 6 as an **opt-in extension that may only follow Level 4 + 5 maturity**.

---

## 2. The fourteen constitutional principles

These principles are constitutional. Every other section of this document is a consequence of them. A system that abandons any one of them is no longer a faithful instance of this design.

**The seven principles of the single-repository society:**

1. **A branch is a thought.** A branch is the natural physical substrate for an insulated cognitive attempt.
2. **A pull request is a settlement.** Disagreement is resolved in the open, on the PR thread, as a transframe with provenance, unknowns, and blind spots.
3. **A merge is an action.** Nothing is "done" until it has merged through the settlement loop.
4. **Memory is committed history.** All durable cognitive artifacts live as files on `main`, with full audit history.
5. **Censors are branch protection.** Required status checks enforce what may not pass, fail-closed, at the boundary.
6. **Critics are graduated.** Reviews, checks, and objections form a layered defence — from cheap heuristic to expensive deliberation.
7. **The B-brain reads patterns, not content.** Meta-observers watch the shape of the cognitive process; they do not read settlement payloads.

**The seven principles of the federation:**

8. **A repository is an agency.** Insulation between agencies is insulation between repositories.
9. **A channel is a settlement.** No cross-boundary act is exempt from the loop.
10. **A bridge is an agency, not a utility.** Lossy translation lives in a repository with a constitution and round-trip tests.
11. **A federation settlement spans repositories.** One settlement ID, many PRs, one closure.
12. **The ledger is append-only.** Corrections are reversing entries. History is never rewritten.
13. **Reputation is a consequence of credit assignment, not an opinion.**
14. **The economic layer is last, never first.** Speed without reflection accelerates mistakes.

These fourteen principles are the constitution. Everything below is mechanism.

---

## 3. The reflective discipline

Below the principles, and above the mechanism, sits a discipline that governs how the society thinks. It applies inside every repository and is repeated across the federation:

- **Frames before K-lines.** A situation is first typed (frame selected, slots filled, defaults marked) before any K-line activates a coordinated response.
- **Recognition before reconstruction.** Recognising a past situation is cheaper than reconstructing the world; recognition must be tried first and its confidence recorded.
- **Representation discipline for every durable artifact.** Every memory artifact declares its `representation_class`; representation is a political choice, not a coincidence.
- **Settlements as transframes with provenance, unknowns, and blind spots.** A settlement names what changed in the situation, what was used to decide, what remained unknown, and what could have biased the decision.
- **Runtime windows with fail-closed behaviour.** Critic windows, censor windows, consolidation windows, dispute windows — each has a duration; missing inputs fail closed.
- **Attention and resource budgets.** Every settlement carries a budget for time, calls, cost, and bridges. Exceeding a budget is a settlement-failing event, not a silent retry.
- **Consolidation windows before memory promotion.** No memory becomes durable without a consolidation window in which late objections and credit-assignment records may arrive.
- **Full-loop credit assignment.** Credit flows back to perception, recognition, frame choice, K-line activation, critic, censor, suppressor, settlement choice, execution, and memory promotion — not only to the visible actor.
- **Differentiation before retirement.** A pressured agency is differentiated by fork under bootstrap protection before any retirement decision.
- **Concept formation with examples and non-examples.** A concept is admitted only when both positive and negative cases are recorded.
- **Plural self-models, slow self-ideals.** A repository carries multiple self-models (as-seen-by-self, as-seen-by-critics, as-seen-by-users) and a small set of slow-changing self-ideals.
- **Plural B-brains that observe but cannot merge.** Meta-observers are themselves plural and can only propose; they cannot act.

The mechanism described below assumes this discipline. Without it, the mechanism becomes ceremony.

---

## 4. Minsky's vocabulary on GitHub primitives

Every concept from the Society of Mind is mapped onto a concrete forge primitive, both within a single repository and across the federation.

| SoM / SOR concept | Within one repository | Across the federation |
| --- | --- | --- |
...
| Frame | `memory/frames/<frame>.yaml` | Frames may be *imported* under provenance from a memory repo via a pinned release |
...
| Action (merge) | Merge into `main` of the acting repo | Also a published release, a service response posted back through a channel, or a signed status update |
...
| Suppressor | Output-boundary catch | Also channel-boundary suppressors: blocks outbound payloads the upstream cloud-egress censor missed |
...
| Constitution | `constitution.yaml` at the agency and society roots | Plus an organisation constitution under a governance repository |
...
The loop is the same shape inside every repository, with the federation arc added when the repository decides delegation is needed.
...
The decisive ordering rules:

- **Delegation is itself a settlement**, not a code path.
...
A federation distinguishes the following roles. They are *roles*, not exclusive types. A small federation may pack several into one repository. A large federation will not.
...
| **Workspace repo** | Holds the federation's current focus and active settlements | Federation settlement issues and `active-settlements/<id>.yaml` files | Run agencies; it points at them |
...
The minimum viable federation is three repositories:

```text
github.com/<org>/
├── <name>-intelligence    ← human doorway + governance + command surface
├── <name>-society         ← the cognitive society
...
| `<name>-society` | Cognitive society | The full reflective stack: frames, K-lines, analogies, agencies, critics, censors, suppressors, memory, workspace, plural self-models and self-ideals | Be installed by hand; it is materialised by the intelligence repo |
...
A Society Channel is a governed cognitive transaction between two repositories (typically across organisations, but allowed within an organisation when insulation is the point). Every stage is a GitHub artifact, not an out-of-band API call.
...
sequenceDiagram
  ...
  participant PS as Provider service<br/>(local loop)
...
| Provider deliberation | Full local loop inside the provider repo | The provider may itself delegate, recursively, with depth limits |
...
A bridge is a translator. Bridges are directional, lossy, learned, and a frequent point of silent failure. A bridge that crosses two societies must be a **repository of its own**, owned by neither side, with:
...
A federation settlement is a **thread of settlements**, one per participating repo, bound together by a shared settlement ID.
...
Insulation is a federation rule.
...
This is exactly the property `git submodule` and dependency pinning give us — this design simply admits that those primitives are the federation's insulation mechanism, not a build-tooling detail.
...
### 12.3 The human anchor

CODEOWNERS at each repository covers that repository's law. A federation also needs a human anchor for *federation* law, placed in the governance repo:
...
Self-ideals are the federation's slow norms. An illustrative starting set:
...
Censors sit on branch protection inside a repository and at the channel boundary between repositories.
...
| `censor/credential` | Required check on every PR in every repo | Any commit containing credentials, tokens, or keys |
...
Fail-closed semantics, with one strengthening at the channel layer:
...
A suppressor is a boundary catch that names the upstream censor that should have prevented the path earlier. Federation-level suppressors sit at the channel boundary itself:
...
Memory inside a repository lives under `memory/**` on `main`, with declared representation classes, temperatures, links, consolidation windows, and concept candidates. Memory across repositories is *published, signed, version-pinned releases* that other repos may import under their own settlement.
...
## 16. Credit assignment, local and federated

Credit flows across the whole loop, both local and cross-society.

```text
local credit targets:
  perception quality
  recognition quality
  frame choice
  analogy choice
...
The federation may eventually monetise. It may not start there. The hard ordering rule:
...
Every ledger entry is append-only. Corrections are reversing entries, never overwrites.
...
A federation that turns on the economic layer before it can demonstrate full reflective discipline, full channel governance, and full audit history is not a Level 6 society. It is a Level 0 marketplace pretending to be a mind. Speed in such a setting accelerates mistakes, not learning.

---

## 18. B-brains and ecology

A B-brain observes the patterns of cognition without reading content. B-brains are plural inside each repository, and plural again at the federation level:
...
This report is reviewed by humans on a fixed cadence: post-outcome reinforcement after every settlement, quarterly ecology review, annual constitution review.

---

## 19. Cognitive observability

Observability signals are emitted by ordinary workflow steps writing to a small set of YAML files in each repository's `state/observability/` directory. The federation B-brain rolls them up into the quarterly ecology report. No external observability stack is required; the substrate is again the repository itself.
...
These boundaries are kept explicit so the federation does not confuse operational connectivity, cognitive depth, and economic activity.
...
│       └── state/
│
├── <name>-society/                             ← cognitive society repo
│   ├── .github/workflows/
│   │   ├── perceive.yml
│   │   ├── activate.yml
│   │   ├── critique.yml
...
The first three (`-intelligence`, `-society`, `-labour`) are required for any federation.
...
## 23. Anti-patterns this design must prevent
...
- **Economic acceleration before cognition** — turning on payments, metering, or reputation before reflective discipline is verifiable.
...
If a federation drifts into any of these, it may still automate work, but it is no longer a faithful instance of this design.

---

## 24. The maturity contract

To claim compliance, a federation must demonstrate:

- [ ] every participating repository has a `constitution.yaml`, CODEOWNERS, branch protection, required censor checks, and reflective discipline (frames, K-lines, analogies, settlements, suppressors, consolidation windows, credit assignment, plural self-models, slow self-ideals, plural B-brains);
...
If these do not hold, the system may still be useful, but it is not yet a faithful instance of this design.
...
2. **Event normalisation, perception, recognition, frame selection**: Frame `frame.contract-renewal` selected with confidence 0.83. Recognition reports 0.78 to past supplier-renewal episodes.
...
11. **Provider deliberation**: each provider runs a full local loop, settles, and dispatches signed responses back. `finance-society` returns a price-history release. `dental-compliance-society` returns a regulatory-event evaluation release.
...
## 26. What this design deliberately does *not* do

- **Open federation discovery without governance.** A society does not discover and call arbitrary other societies on the public internet. Discovery is bounded to the service registry pinned by the federation governance repo. Cross-federation discovery is a future design.
- **Automatic constitution import.** A society does not inherit another society's constitution wholesale. Constitutions are written and reviewed locally.
- **Automatic bridge synthesis.** Bridges are written and reviewed by humans (or, eventually, by a bridge-authoring agency under strict bootstrap). Translators are not auto-generated between vocabularies.
...
- **Public-currency settlement on the ledger.** The ledger shape is specified but not bound to a real currency. Implementations may stay in credits / reciprocal until governance opens a real currency channel.
...
These are not failures of ambition. They are explicit choices to keep the architecture honestly Minskyan: many small, limited, inspectable parts; insulation as fundamental as connection; representation as a political choice; no compromise between equals.

---

## 27. What this design does not claim
...
A federation is built in ordered stages. Skipping a stage is not faster; it is more brittle, and the first surprise is more expensive.
...
### Stage 1 — one reflective society

- `<name>-society` reaches full reflective compliance: frames, K-lines, analogies, settlements, suppressors, consolidation windows, full-loop credit assignment, plural self-models, slow self-ideals, plural B-brains;
...
- the maturity contract above (§24) is verified;
...
## 29. Closing — the architecture in one breath
...
The architecture adds the missing machinery:

- repository roles and the three-repo minimum federation;
- the Society Channel Protocol on GitHub primitives;
...
- federation credit assignment and reputation as its consequence;
- the ledger, last;
...
A repo can think. A repo can learn what kind of thinker it is becoming. Many such repos can speak — and the only honest way for them to speak is to hold the act of speaking to the same standard as the act of thinking.

---

*Source material: [`research/THE-SOCIETY-OF-MIND/`](research/THE-SOCIETY-OF-MIND/), [`research/THE-REPO-IS-SOCIETY/`](research/THE-REPO-IS-SOCIETY/), [`research/THE-SOCIETY-OF-REPO/`](research/THE-SOCIETY-OF-REPO/) (especially [`02-protocols/07-service-channel.md`](research/THE-SOCIETY-OF-REPO/02-protocols/07-service-channel.md), [`02-protocols/18-bridges.md`](research/THE-SOCIETY-OF-REPO/02-protocols/18-bridges.md), [`08-services/`](research/THE-SOCIETY-OF-REPO/08-services/), [`09-channels/`](research/THE-SOCIETY-OF-REPO/09-channels/), [`10-evolution/`](research/THE-SOCIETY-OF-REPO/10-evolution/), [`00-foundations/03-maturity-model.md`](research/THE-SOCIETY-OF-REPO/00-foundations/03-maturity-model.md), [`00-foundations/09-cognitive-observability.md`](research/THE-SOCIETY-OF-REPO/00-foundations/09-cognitive-observability.md)), and [`research/FORGEJO-SOCIETY-IMPLEMENTATION/`](research/FORGEJO-SOCIETY-IMPLEMENTATION/) (especially [`13-inter-repo-communication.md`](research/FORGEJO-SOCIETY-IMPLEMENTATION/13-inter-repo-communication.md) and [`14-three-repo-implementation-targets.md`](research/FORGEJO-SOCIETY-IMPLEMENTATION/14-three-repo-implementation-targets.md)).*
