...
aroused certain other agents. So let's take *reward* to mean
...
through a *reward* system that, when activated after each
...
reuse its agents in different ways in different contexts — as See
...
something we liked, both those decisions became more likely — and
all those *goods* and *bads* tended to cancel one another
out!

This poses a dilemma in designing machines that learn
by *reinforcing* the connections between agents. In the course of
solving a hard problem, one will usually try several bad moves before
finding a good one — for this is virtually what we mean by calling
a problem *hard.* To avoid learning those bad moves, we could
...
[« Previous](som-7.5.md) | [Contents](contents.md) | [Next »](som-7.7.md)
