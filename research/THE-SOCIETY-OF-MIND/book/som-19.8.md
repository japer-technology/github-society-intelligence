...
all dogs of similar size — or, even, animals of every
...
down the same street again — but it never stopped chasing cars
on other streets.
...
like *abstraction,* *induction,*
*abduction,* and so forth. But no one has found a way to make
consistently correct generalizations — presumably because no
such foolproof scheme exists, and whatever we *learn* may
...
concept of a
*level-band.* Yet another scheme is implicit in the concept
...
we *represent* what we already know and the generalizations
that will seem most plausible. For example, when we first proposed
a *recognizer* for a chair, we composed it from the polynemes
for several already familiar ideas, namely seats, legs, and
backs. We gave these features certain weights.
...
for *back,* the new agent would reject chairs but would
...
hidden from view — as well as other objects that weren't
furniture at all.
...
[« Previous](som-19.7.md) | [Contents](contents.md) | [Next »](som-19.9.md)
