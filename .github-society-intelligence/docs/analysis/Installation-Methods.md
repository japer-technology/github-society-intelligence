...
**Question:** What mechanisms could be implemented in [`japer-technology/github-society-intelligence`](https://github.com/japer-technology/github-society-intelligence) to facilitate automatically copying `.github/workflows/github-society-intelligence-agent.yml` into consuming repositories?

**Target file:** `https://github.com/japer-technology/github-society-intelligence/.github/workflows/github-society-intelligence-agent.yml`
**Destination:** `.github/workflows/github-society-intelligence-agent.yml` in the consuming repo
...
1. User manually copies `github-society-intelligence-agent.yml` into their repo's `.github/workflows/` directory
2. User triggers the workflow via `workflow_dispatch`
3. The workflow's `install` job downloads the rest of the framework (`.github-society-intelligence/` folder) from the source repo via `curl` + ZIP extraction
...
mkdir -p .github/workflows && curl -sL \
  "https://raw.githubusercontent.com/japer-technology/github-society-intelligence/main/.github/workflows/github-society-intelligence-agent.yml" \
  -o .github/workflows/github-society-intelligence-agent.yml
```

Or with a hosted install script:
```bash
curl -sL https://raw.githubusercontent.com/japer-technology/github-society-intelligence/main/install.sh | bash
```
...
**What to add to the source repo:** A `gh` CLI extension (e.g., `gh-gsi`) that copies the workflow file into the current repo.

**How it works:**
```bash
gh extension install japer-technology/gh-gsi
gh gsi install
```

The extension would be a separate repo (e.g., `japer-technology/gh-gsi`) containing a shell script or Go binary that:
...
- New repo is created with all files from the template, including `.github/workflows/github-society-intelligence-agent.yml`
...
**Cons:** Only works for **new** repos (not existing ones like `gsi`), copies entire repo not just the workflow, no update mechanism
...
```yaml
# .github/workflows/github-society-intelligence-agent.yml (in consuming repo)
name: github-society-intelligence-agent
on:
  issues:
    types: [opened]
  issue_comment:
...
jobs:
  agent:
    uses: japer-technology/github-society-intelligence/.github/workflows/github-society-intelligence-agent.yml@main
    secrets: inherit
```
...
└── workflow-templates/
    ...
    └── github-society-intelligence-agent.properties.json
```
...
**How it works:**
1. Register a GitHub App (e.g., "GSI Installer")
2. When installed on a repo, the app:
   - Creates a PR adding `.github/workflows/github-society-intelligence-agent.yml`
...
**How it works:**
```yaml
# In github-society-intelligence repo
  ...
  distribute:
    ...
    steps:
      ...
      - name: Copy workflow to target
        ...
        run: |
          gh api repos/${{ inputs.target_repo }}/contents/.github/workflows/github-society-intelligence-agent.yml \
            --method PUT \
            -f message="chore: install github-society-intelligence workflow" \
            -f content="$(base64 -w0 .github/workflows/github-society-intelligence-agent.yml)"
```
...
```bash
git submodule add https://github.com/japer-technology/github-society-intelligence.git .gsi-upstream
# Then symlink or copy the workflow file
cp .gsi-upstream/.github/workflows/github-society-intelligence-agent.yml .github/workflows/
```
...
**How it works in the consuming repo:**
```bash
git subtree add --prefix=.gsi-upstream \
  https://github.com/japer-technology/github-society-intelligence.git main --squash
cp .gsi-upstream/.github/workflows/github-society-intelligence-agent.yml .github/workflows/
```

Update:
```bash
git subtree pull --prefix=.gsi-upstream \
  https://github.com/japer-technology/github-society-intelligence.git main --squash
```
...
**How it works:**
```bash
npm install @japer-technology/github-society-intelligence --save-dev
...
```json
{
  "name": "@japer-technology/github-society-intelligence",
  "scripts": {
    "postinstall": "mkdir -p ../../.github/workflows && cp workflow.yml ../../.github/workflows/github-society-intelligence-agent.yml"
  }
}
```
...
gh release download --repo japer-technology/github-society-intelligence \
  --pattern "github-society-intelligence-agent.yml" \
...
```yaml
# .github/workflows/setup-gsi.yml
name: Setup GSI
...
jobs:
  setup:
    ...
    steps:
      - uses: actions/checkout@v4
      - uses: japer-technology/github-society-intelligence@main
        with:
          action: install-workflow
      - run: |
          git add .github/workflows/github-society-intelligence-agent.yml
          git commit -m "chore: install GSI workflow"
          git push
```
...
# Using Copier
pipx install copier
copier copy gh:japer-technology/github-society-intelligence my-project

# Using Cookiecutter
pipx install cookiecutter
cookiecutter gh:japer-technology/github-society-intelligence
```
...
**How it works in the consuming repo:**
```yaml
# .github/workflows/sync-gsi-workflow.yml
name: Sync GSI Workflow
on:
  schedule:
    - cron: '0 0 * * 1'  # Weekly
  workflow_dispatch:
  ...
  sync:
    ...
    steps:
      ...
      - name: Download latest workflow
        run: |
          curl -sL "https://raw.githubusercontent.com/japer-technology/github-society-intelligence/main/.github/workflows/github-society-intelligence-agent.yml" \
            -o .github/workflows/github-society-intelligence-agent.yml
      - name: Commit if changed
        run: |
          git diff --quiet || (git add . && git commit -m "chore: sync GSI workflow" && git push)
```
...
jobs:
  update-consumers:
    strategy:
      matrix:
        repo: [japer-technology/gsi, japer-technology/other-repo]
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
        with:
      ...
      - uses: peter-evans/create-pull-request@v6
        with:
          token: ${{ secrets.CROSS_REPO_PAT }}
          commit-message: "chore: update GSI workflow to latest"
          title: "Update github-society-intelligence-agent workflow"
          branch: update-gsi-workflow
```
...
1. Source repo publishes a `latest.json` (e.g., via GitHub Pages or as a release):
   ...
   {
     "version": "1.0.1",
     "workflow_url": "https://raw.githubusercontent.com/.../github-society-intelligence-agent.yml",
     "sha256": "abc123..."
   }
   ```
...
5. **Provide a sync workflow template** (Method 14) — Ship a `sync-gsi-workflow.yml` template that consuming repos can add alongside the main workflow. This gives auto-updates with minimal source repo changes.
