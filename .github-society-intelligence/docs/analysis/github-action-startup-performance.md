...
| 6 | **Install** | `bun install --frozen-lockfile` in `.github-society-intelligence/` | 0–5s (cache hit) / 10–30s (cache miss) |
...
| `sparse-checkout` with only `.github-society-intelligence/` | Saves time if repo has many large files | ❌ Breaks when `pi` agent reads/edits repo files |
...
**What's happening:**
1. `actions/cache@v5` attempts to restore `.github-society-intelligence/node_modules` using a cache key derived from the hash of `bun.lock`. On a cache hit, the entire `node_modules` directory is restored from GitHub's cache storage.
...
- name: Run
  run: |
    git config user.name "github-society-intelligence[bot]"
    git config user.email "github-society-intelligence[bot]@users.noreply.github.com"
    bun .github-society-intelligence/lifecycle/agent.ts
```
