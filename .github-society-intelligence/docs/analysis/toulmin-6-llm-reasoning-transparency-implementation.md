...
| Commit-level reasoning metadata | **Missing** — commit messages are generic (`society-intelligence: work on issue #N`) | Traceability gap |
...
**Current behavior**: Commit messages are static strings: `society-intelligence: work on issue #<N>`.
...
```
.github-society-intelligence/state/reasoning/.gitkeep
```
...
The workflow at `.github/workflows/github-society-intelligence-agent.yml` does not need modification for this feature. The reasoning capture happens entirely within the agent's TypeScript execution, which the workflow already invokes via `bun .github-society-intelligence/lifecycle/agent.ts`.
...
*Implementation outline derived from [Toulmin Lessons §6](toulmin-lessons.md) and codebase analysis of [GitHub Society Intelligence](https://github.com/japer-technology/github-society-intelligence)*
