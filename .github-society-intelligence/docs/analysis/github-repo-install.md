...
This document describes the **Create and Run Workflow** installation method — the simplest path to installing GitHub Society Intelligence. You add one workflow file to your repository, trigger it, and the workflow installs everything else automatically.

---

## 1. Overview

The entire installation reduces to a single file: `.github/workflows/github-society-intelligence-agent.yml`. Once committed and triggered via `workflow_dispatch`, its `install` job downloads the latest `.github-society-intelligence/` folder from the template repository, commits it to your default branch, and configures the agent to run on future issue and comment events.
...
Create the file `.github/workflows/github-society-intelligence-agent.yml` in your repository.

The source file is maintained in the template repository:
[**View workflow source →**](https://github.com/japer-technology/github-society-intelligence/blob/main/.github/workflows/github-society-intelligence-agent.yml)
...
```
https://github.com/OWNER/REPO/new/main?filename=.github/workflows/github-society-intelligence-agent.yml
```

This opens the GitHub **Create new file** editor with the correct path and filename already filled in. Copy the [workflow source](https://raw.githubusercontent.com/japer-technology/github-society-intelligence/main/.github/workflows/github-society-intelligence-agent.yml), paste it into the editor, and commit directly to your default branch.
...
3. Enter the path: `.github/workflows/github-society-intelligence-agent.yml`
4. Paste the [workflow source](https://raw.githubusercontent.com/japer-technology/github-society-intelligence/main/.github/workflows/github-society-intelligence-agent.yml) into the editor.
...
git commit -m "chore: add github-society-intelligence workflow"
git push
```
...
2. Select **github-society-intelligence-agent** from the workflow list on the left.
...
| **Check for existing install** | Looks for the `.github-society-intelligence/` directory. If it already exists, the job exits — nothing is overwritten |
| **Download template** | Fetches the latest ZIP archive from `github.com/japer-technology/github-society-intelligence/archive/refs/heads/main.zip` |
| **Extract and copy** | Extracts `.github-society-intelligence/` from the archive into the repository root |
...
| **Commit and push** | Commits the new `.github-society-intelligence/` folder and pushes to the default branch |

After this job completes, your repository contains the full `.github-society-intelligence/` directory and the agent is ready to respond to issues.
...
|---|---|---|
| `workflow_dispatch` (manual) | `install` | Downloads and installs `.github-society-intelligence/` if not already present |
...
| `push` to `main` | `deploy-pages` | Deploys static content from `.github-society-intelligence/public-fabric/` to GitHub Pages |
...
6. **Runs the agent** — executes `bun .github-society-intelligence/lifecycle/agent.ts` with all configured API keys.
...
To update to the latest version, the `install` job only runs when `.github-society-intelligence/` does not exist. For updates, use the setup script from the repository root:

```bash
curl -fsSL https://raw.githubusercontent.com/japer-technology/github-society-intelligence/main/.github-society-intelligence/script/setup.sh | bash
```

The script detects the existing installation, compares version numbers (from `.github-society-intelligence/VERSION`), backs up user customisations (`AGENTS.md`, `.pi/`, `state/`), replaces framework files, and restores the backups.
...
| Check | How |
|---|---|
| Install job succeeded | **Actions** tab → the `github-society-intelligence-agent` run shows a green ✅ on the `install` job |
| Files are present | The repository now contains `.github-society-intelligence/VERSION` |
| Workflow is active | **Actions** tab → `github-society-intelligence-agent` appears in the workflow list |
...
The **Create and Run Workflow** method is the most minimal installation path for GitHub Society Intelligence. It requires creating a single file (`.github/workflows/github-society-intelligence-agent.yml`), committing it, and triggering the workflow manually from the Actions tab. The workflow's `install` job downloads the latest `.github-society-intelligence/` folder from the template repository, configures defaults, and commits everything to the default branch — no local tooling, no build steps, no manual file copying.
