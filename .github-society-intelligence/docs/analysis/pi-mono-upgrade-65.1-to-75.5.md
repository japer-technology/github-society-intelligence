...
This document outlines the full scope of changes required to update GitHub Society Intelligence's core dependency — `@mariozechner/pi-coding-agent` — from **v0.65.1** to **v0.75.5**. Beyond the usual version bump, this update also covers the project's relocation to a **new GitHub owner and npm scope**: `badlogic/pi-mono` → `earendil-works/pi`, with packages re-published under `@earendil-works/*`. The release span covers **25 releases** (v0.67.4 through v0.75.5) including 6 releases with breaking changes, supply-chain hardening work, a new minimum Node.js version, and many fixes directly relevant to GSI's non-interactive `--mode json` usage.

| Item | Detail |
|------|--------|
| **Current version** | `0.65.1` (pinned in `.github-society-intelligence/package.json`) |
...
- **v0.73.1** added migration support to `pi update --self`: when the version-check endpoint reports a renamed package, the self-update flow now uninstalls the old global package before installing the new one. GSI does not use `pi update --self` (it pins a specific version in `package.json`), but anyone running pi standalone benefits.
...
### 2.3 Implication for GSI

GSI must change the **package name** in `package.json`, regenerate `bun.lock`, and update **all documentation references** that point at the old repository URL or scoped package name. There are no API/CLI changes that flow from the ownership move itself — only the dependency identifier changes.
...
- Fix: shutdown kills tracked detached bash child processes on exit signals — **relevant for GSI**: avoids orphaned `gh`/`git` subprocesses left over after a JSON-mode run.
...
- **New**: prompt templates support an `argument-hint` frontmatter field for `/` autocomplete dropdowns. **Useful for GSI**: applies to GSI's `.pi/prompts/code-review.md` and `.pi/prompts/issue-triage.md` if they ever get arguments.
...
- Fix: **system prompt dates use stable `YYYY-MM-DD` format** instead of locale-dependent output — **important for GSI**: keeps GitHub Actions agent prompts deterministic across runners.
- Fix: auto-retry treats `Network connection lost.` as retryable — **relevant for GSI**: reduces hard failures on transient network blips in CI.
...
- **⚠️ Breaking — does NOT impact GSI**: SDK `createAgentSession({ tools })` now expects `string[]` tool names instead of `Tool[]` instances. **GSI uses the CLI**, where `--tools read,bash,edit,write,grep,find,ls` is already the comma-separated name-allowlist shape — **no change required**.
- **⚠️ Breaking — does NOT impact GSI**: removed prebuilt cwd-bound tool exports from `@mariozechner/pi-coding-agent` (`readTool`, `bashTool`, …). Use `createReadTool(cwd)` etc. SDK-only.
- **⚠️ Breaking — does NOT impact GSI**: removed ambient `process.cwd()` defaults from `DefaultResourceLoader`, `loadProjectContextFiles()`, `loadSkills()`. SDK-only.
- **⚠️ Breaking — IMPACTS GSI (interactive flag semantics)**: `--no-tools` now disables all tools by default rather than only built-ins. GSI does not pass `--no-tools`, so this is informational.
- Fix: `sessionDir` in `settings.json` now expands `~` — **relevant for GSI** if migrating off `--session-dir` (see §6.5).
- Fix: `edit` tool coerces stringified `edits` JSON before validation — **relevant for GSI**: fewer edit retries when models pass `edits` as a string.
...
- **⚠️ Breaking — IMPACTS GSI**: first-party migration from `@sinclair/typebox` 0.34.x to `typebox` 1.x. New extensions should `import { Type } from "typebox"`. Legacy extension loading still **aliases the root `@sinclair/typebox` package**, so GSI's `github-context.ts` continues to work with its current `import { Type } from "@sinclair/typebox"`. However, the `@sinclair/typebox/compiler` subpath is **no longer shimmed** — not used by GSI. See §6.3 for the optional but recommended cleanup.
- **⚠️ Breaking — does NOT impact GSI**: session-replacement commands invalidate captured pre-replacement extension `pi`/`ctx` references. GSI's extension does not call `ctx.newSession()`, `ctx.fork()`, or `ctx.switchSession()`. **No change required**.

### v0.70.0 — **⚠️ Breaking Change**

- **New**: searchable auth provider login flow (interactive `/login`).
- **New**: GPT-5.5 Codex (`openai-codex/gpt-5.5`) with `xhigh` reasoning. **Relevant for GSI**: GSI's `settings.json` is configured for `openai`/`gpt-5.5`/`high`; this release is when GPT-5.5 Codex variants were added.
...
- **⚠️ Breaking — does NOT impact GSI**: OSC 9;4 terminal progress indicators disabled by default (interactive cosmetic).
...
- **New**: `pi update` can now update pi itself in addition to installed pi packages (interactive convenience; GSI pins versions).
...
- Fix: bash executor temp output streams no longer leak file descriptors when output is truncated by line count — **relevant for GSI**: long-running `bun test` or `gh` invocations.
...
- Fix: file discovery falls back to `fdfind` when `fd` is unavailable — **relevant for GSI**: Debian/Ubuntu GitHub Actions runners that don't ship `fd`.

### v0.71.0 — **⚠️ Breaking Change**

- **⚠️ Breaking — does NOT impact GSI**: removed built-in Google Gemini CLI and Google Antigravity provider support. GSI uses `openai`, so unaffected. Anyone configured for `google-gemini-cli` or `google-antigravity` must migrate.
...
- **New**: `PI_CODING_AGENT_SESSION_DIR` env var as an equivalent to `--session-dir` — **directly relevant for GSI**: could replace the `--session-dir` CLI flag in `agent.ts` (see §6.5).
...
- **⚠️ Breaking — does NOT impact GSI**: `compat.reasoningEffortMap` in `models.json` and `pi.registerProvider()` replaced by model-level `thinkingLevelMap`. GSI does not ship a `models.json` or call `registerProvider`. **No change required**.
...
- **New**: **incremental bash output streaming** — bash tool output now appears while commands run instead of only after completion. **Relevant for GSI**: improves visibility in JSON-mode for long-running commands.
- **New**: compact `read` rendering for Pi docs / context files / skills in interactive mode.
- **⚠️ Breaking — does NOT impact GSI**: built-in `xiaomi` provider switched from Token Plan AMS to Xiaomi's API billing endpoint; affected users must switch to `xiaomi-token-plan-*` providers. GSI uses `openai`.
...
- Fix: OpenAI Codex WebSocket transport keeping `--print` and **JSON mode processes alive after the response** by closing cached WebSocket sessions during session shutdown — **critical for GSI** when Codex transports are used: prevents agent processes from hanging in CI runs.
- Fix: interactive sessions now exit when terminal input is lost.

### v0.73.1

- **New**: self-update support for the **npm scope migration** (`@mariozechner/pi-coding-agent` → `@earendil-works/pi-coding-agent`) via `pi update --self`. GSI pins versions and does not use this path.
...
- Fix: `pi -p` no longer treats prompts that start with YAML frontmatter as extension flags — **relevant for GSI**: GSI's prompts in `.pi/prompts/*.md` start with YAML frontmatter; if a future invocation pipes a templated prompt body to `-p`, this fix prevents misinterpretation.
...
- Fix: `--resume` session listing caps in-flight session metadata loads — avoids OOM on large session histories. **Relevant for GSI**: long-lived issue threads accumulate sessions.
...
- **⚠️ Breaking — VERIFY for GSI**: **minimum Node.js raised to 22.19.0**. **Action required**: confirm the GitHub Actions workflow uses a Node toolchain that satisfies this floor (or relies entirely on Bun's bundled runtime, in which case this is a moot point for runtime, but `pi update --self` / npm installs of pi packages still need Node 22.19+). See §6.6.
- Fix: compaction summary calls use custom agent stream functions, preserving proxy-backed LLM routing.
- Fix: **system prompt and context file boundaries use explicit XML tags instead of Markdown headings** — reduces inconsistent boundary ingestion by models. **Relevant for GSI**: improves how `AGENTS.md` and `.pi/APPEND_SYSTEM.md` are delimited in the assembled system prompt.
...
- Fix: AgentSession retry, compaction, and event settlement use the awaited agent lifecycle; added `willRetry` to `agent_end` session events — **relevant for GSI**: post-run JSONL consumers can now reliably detect retry-in-progress vs. terminal end states.
...
- Fix: bash tool truncation line counts ignore the trailing newline as an extra output line — **relevant for GSI**: more accurate truncation messaging on `gh` / `bun test` output.

---

## 4. Breaking Changes Impact Assessment for GSI

### 4.1 IMPACTS GSI — npm Scope and GitHub Org Migration (v0.73.1 / v0.74.0)

**What changed**: `@mariozechner/pi-coding-agent` → `@earendil-works/pi-coding-agent`; `badlogic/pi-mono` → `earendil-works/pi`.

**Impact on GSI**: The dependency name in `.github-society-intelligence/package.json` must change, the lockfile must be regenerated, and every documentation reference that names the old scope or links to `github.com/badlogic/pi-mono` must be updated.

**Required change**: see §6.1, §6.2, §7.

### 4.2 IMPACTS GSI — Node.js Minimum Raised to 22.19.0 (v0.75.0)

**What changed**: pi requires Node.js >= 22.19.0.

**Impact on GSI**: GSI's lifecycle runs `pi` as a child of `Bun.spawn(...)` (see `lifecycle/agent.ts:331`). Bun ships its own runtime, but **pi itself is invoked via the `node_modules/.bin/pi` shim, which is a Node script** — so a Node 22.19+ install must be present on the runner. The standard GitHub-hosted `ubuntu-latest` image ships Node >= 22 in 2026, but workflows that pin Node 20 (or use `actions/setup-node@v4` to install an older version) will break. See §6.6 for the verification step.

### 4.3 DOES NOT IMPACT GSI — SDK Tool-Array Shape Change (v0.68.0)

**What changed**: SDK `createAgentSession({ tools })` now expects `string[]` of tool names.

**Impact on GSI**: GSI uses the CLI, and `--tools read,bash,edit,write,grep,find,ls` is already a comma-separated name list. **No change required**.

### 4.4 DOES NOT IMPACT GSI — Prebuilt cwd-bound Tool Exports Removed (v0.68.0)
...
**Impact on GSI**: GSI's `github-context.ts` extension does not import any of these. **No change required**.

### 4.5 DOES NOT IMPACT GSI — Ambient cwd Removed From Resource Helpers (v0.68.0)

**What changed**: `DefaultResourceLoader`, `loadProjectContextFiles()`, `loadSkills()` require explicit cwd/agent-dir inputs.

**Impact on GSI**: GSI's extension does not call any of these. **No change required**.

### 4.6 DOES NOT IMPACT GSI — TypeBox 1.x Migration (v0.69.0)
...
**Impact on GSI**: GSI's `.pi/extensions/github-context.ts` does `import { Type } from "@sinclair/typebox"` and **does not import from `@sinclair/typebox/compiler`**. The alias keeps it working. **No mandatory change required**, but switching the import to `typebox` is the long-term direction (see §6.3).

### 4.7 DOES NOT IMPACT GSI — Session-Replacement Invalidation (v0.69.0)
...
**Impact on GSI**: GSI's extension does not call any session-replacement APIs. **No change required**.

### 4.8 DOES NOT IMPACT GSI — Google Gemini CLI / Antigravity Removed (v0.71.0)

**What changed**: Removed built-in `google-gemini-cli` and `google-antigravity` provider support.

**Impact on GSI**: GSI defaults to `openai`/`gpt-5.5`. **No change required**.

### 4.9 DOES NOT IMPACT GSI — `reasoningEffortMap` → `thinkingLevelMap` (v0.72.0)
...
**Impact on GSI**: GSI does not ship a `models.json` or register custom providers. **No change required**.

### 4.10 DOES NOT IMPACT GSI — Xiaomi Provider Endpoint Switch (v0.73.0)
...
**Impact on GSI**: GSI uses `openai`. **No change required**.

### 4.11 DOES NOT IMPACT GSI — OSC 9;4 Terminal Progress Default-Off (v0.70.0)

**What changed**: OSC 9;4 progress indicators default to off; controlled by `terminal.showTerminalProgress`.

**Impact on GSI**: Interactive cosmetic. JSON mode does not emit OSC 9;4. **No change required**.

---

## 5. Bug Fixes and Features Most Relevant to GSI

These fixes/features directly address issues GSI has experienced or is susceptible to in its `--mode json` / GitHub Actions usage:
...
The biggest user-visible runtime change. Prior to v0.73.0, `bash` tool output appeared only when the command finished; GSI runs that invoked long-running `bun test`, `gh pr checks`, or large `git log` operations could appear to hang. From v0.73.0 onward, output streams incrementally into the JSONL transcript.

### 5.2 Highlight: XML-Tagged System Prompt Boundaries (v0.75.0)

The default system prompt now wraps context files (`AGENTS.md`, `CLAUDE.md`) and skill contents in explicit XML tags rather than Markdown headings. This reduces a long-standing failure mode where models would treat a context-file section heading as an instruction boundary or confuse a Markdown heading inside a context file with a system-prompt section. GSI's `AGENTS.md` and `.pi/APPEND_SYSTEM.md` benefit automatically.
...
**File**: `.github-society-intelligence/package.json`
...
**File**: `.github-society-intelligence/lifecycle/agent.ts` (lines ~310–329)

All flags currently used by GSI remain valid in v0.75.5:
...
v0.71.0 added `PI_CODING_AGENT_SESSION_DIR` as an environment-variable equivalent to the `--session-dir` flag. Combined with v0.68.1's `~` expansion in `sessionDir`, GSI could move session-directory config out of `agent.ts` entirely. Recommend deferring to a separate PR — the current path is computed at runtime relative to cwd, so any move needs explicit testing of relative-path resolution semantics.
...
**File**: `.github-society-intelligence/PACKAGES.md` (line ~12)
...
**File**: `.github-society-intelligence/docs/security-assessment.md`
...
**File**: `.github-society-intelligence/public-fabric/status.json`
...
**File**: `.github-society-intelligence/docs/analysis/pi-mono-feature-utilization.md`
...
1. `cd .github-society-intelligence && bun install --frozen-lockfile` — confirm the current `0.65.1` lock is valid.
...
2. Run `cd .github-society-intelligence && bun install` to regenerate `bun.lock`.
...
1. `cd .github-society-intelligence && node_modules/.bin/pi --version` — expect `0.75.5`.
2. Smoke test:
   ```
   pi --mode json \
...
7. Trigger a real issue comment in a test repository — verify the full GSI lifecycle.
...
| `@earendil-works/pi-coding-agent` not yet published when GSI updates | Very Low | High | Verify availability on npm before merging the version bump |
...
The update is entirely contained within `.github-society-intelligence/` and does not affect repository structure or workflow logic. Note that **`@mariozechner/pi-coding-agent` is deprecated upstream** — rollback should be considered a temporary measure, not a steady state.
...
The update from v0.65.1 to v0.75.5 brings **25 releases** of improvements alongside the project's **migration from `badlogic/pi-mono` (`@mariozechner/*`) to `earendil-works/pi` (`@earendil-works/*`)**. The most impactful changes for GSI are:
...
- **TypeBox 1.x** (v0.69.0) — legacy alias keeps GSI's current import working; migration is optional but recommended.
...
All other breaking changes either don't affect GSI's CLI-based usage or are gated behind legacy aliases. The update is moderate-risk (primarily the package rename and Node version floor) with substantial reward — it addresses several CI-relevant reliability issues and brings GSI onto the maintained `@earendil-works/*` track.
