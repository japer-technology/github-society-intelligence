...
- Remove `.github-society-intelligence/`, and the repo-local agent framework is gone.
...
5. Wait for the workflow to commit `.github-society-intelligence/` into the repo
6. Open an issue
...
- downloads the latest `.github-society-intelligence` folder from the template repo
...
> Note: the default installed settings point to OpenAI. If you use another provider, install first, then edit `.github-society-intelligence/.pi/settings.json` and commit the change.
...
jobs:
  install:
    ...
    steps:
      ...
      - name: Check for .github-society-intelligence
        id: check-folder
        run: |
          if [ -d ".github-society-intelligence" ]; then
            echo "exists=true" >> "$GITHUB_OUTPUT"
            echo "✅ .github-society-intelligence already exists — nothing to do."
          else
      ...
      - name: Commit and push
        ...
        run: |
          ...
          git add .github-society-intelligence/

          if git diff --cached --quiet; then
            echo "No changes to commit."
          else
            git commit -m "chore: install .github-society-intelligence from template"
            git push
          fi
  ...
  run-agent:
    runs-on: ubuntu-latest
    concurrency:
      group: github-society-intelligence-${{ github.repository }}-issue-${{ github.event.issue.number }}
    ...
    steps:
      ...
      - name: Check for .github-society-intelligence
        ...
        run: |
          if [ -d ".github-society-intelligence" ]; then
            ...
            echo "exists=false" >> "$GITHUB_OUTPUT"
            echo "::notice::.github-society-intelligence folder not found, skipping."
          fi
      ...
      - name: Cache dependencies
        ...
        with:
          path: .github-society-intelligence/node_modules
          key: si-deps-${{ hashFiles('.github-society-intelligence/bun.lock') }}

      - name: Install dependencies
        if: steps.check-folder.outputs.exists == 'true'
        run: cd .github-society-intelligence && bun install --frozen-lockfile
      ...
      - name: Run
        ...
        env:
          ...
          GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
        run: bun .github-society-intelligence/lifecycle/agent.ts
```
...
2. Click **Actions**
3. Open **github-society-intelligence-agent**
...
- create `.github-society-intelligence/`
...
```text
chore: install .github-society-intelligence from template
```
...
```text
.github-society-intelligence/AGENTS.md
.github-society-intelligence/.pi/settings.json
.github-society-intelligence/lifecycle/agent.ts
.github-society-intelligence/install/
.github-society-intelligence/docs/
```
...
### Files
- [ ] `.github/workflows/github-society-intelligence-agent.yml` exists
- [ ] `.github-society-intelligence/` exists after the manual run
- [ ] `.github-society-intelligence/.pi/settings.json` exists
- [ ] `.github-society-intelligence/AGENTS.md` exists
...
- the file path is exactly `.github/workflows/github-society-intelligence-agent.yml`
...
- the repo already had `.github-society-intelligence/`

## 3. The agent does not respond to issues
Check:

- `.github-society-intelligence/` exists in the repo
- a provider secret exists
- `.github-society-intelligence/.pi/settings.json` matches that provider
...
- review the installed `.github-society-intelligence/` folder
