...
This document details the implementation plan for enabling pi's additional built-in tools (`grep`, `find`, `ls`) in the GSI agent, as identified in the [implementation plan](implementation-plan.md).
...
find . -name "*.ts" -type f
grep -rn "TODO" src/
ls -la .github-society-intelligence/
```
