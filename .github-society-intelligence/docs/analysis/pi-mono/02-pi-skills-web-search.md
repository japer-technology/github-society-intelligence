...
This document details the implementation plan for installing the `brave-search` skill from [pi-skills](https://github.com/badlogic/pi-skills) to give the GSI agent web research capability, as identified in the [implementation plan](implementation-plan.md).
...
| Skill | Description | Applicability to GSI |
|---|---|---|
...
- **Free tier:** 2,000 queries/month (sufficient for most GSI workloads)
...
Update `.github/workflows/github-society-intelligence-agent.yml` to pass the key:
...
- name: Install brave-search dependencies
  run: |
    cd .github-society-intelligence/.pi/skills/brave-search
    npm install
```
...
The free tier is sufficient for all expected GSI usage patterns.
...
Installing the `brave-search` skill from pi-skills gives the GSI agent web research capability with minimal effort. The skill uses pi's existing project-local skill discovery mechanism, requires only a free Brave Search API key, and aligns with the "GitHub as Infrastructure" principle when committed to the repository. The free API tier supports up to 2,000 queries/month, well within expected usage.
