...
The core tension: GSI's "GitHub as Infrastructure" principle means the canonical interaction surface is GitHub Issues. But there are UX needs that Issues cannot serve — configuration, visualisation, onboarding, diagnostics, and offline tooling. This brainstorm explores how GitHub Pages can bridge that gap without violating the principle.
...
**Concept:** An interactive documentation site that visualises GSI's architecture — file structure, workflow triggers, event flow, state management — with clickable diagrams that link to the actual source files in the repository.
...
**Concept:** Serve the `setup.sh` installer script from GitHub Pages so users can install GSI with a single `curl | bash` command pointing to the Pages URL.
...
**Value:** AGENTS.md is the most important configuration file in GSI, but writing one from scratch is daunting. A guided composer makes the structure visible and the process approachable.
...
These reinforce GSI's commitment to transparency and institutional trust.
...
These reduce the ongoing cost of operating GSI.
...
All other ideas are self-contained tools that use standard HTML, CSS, and JavaScript. This means GSI can pursue the majority of these ideas without taking a dependency on pi-web-ui — the dependency question is isolated to the session-related features.
...
*Brainstorm based on GSI's architecture, public-fabric deployment, and the principle that a UX making GitHub's "programmerville" disappear is a good thing. As of 2026-03-30.*
