...
This document outlines the concrete changes required to implement Recommendation 4 from [Toulmin Lessons](toulmin-lessons.md) — mapping each security finding to the Toulmin claim it rebuts and hardening GSI accordingly. Four security findings are in scope: SEC-008 (workflow self-replication), SEC-002 (unrestricted network egress), SEC-005 (no branch protection), and SEC-003 (token scope).
...
| `.github-society-intelligence/lifecycle/agent.ts` | Prevents self-modification of the agent runtime |

Store the blocked-path list in a configuration file (e.g., `.github-society-intelligence/config/restricted-paths.json`) so it can be updated without modifying `agent.ts` itself.
...
```
society-intelligence: work on issue #42
...
**File**: `.github/workflows/github-society-intelligence-agent.yml`, within the `run-agent` job definition.
...
- **Network policy documentation**: Add a file at `.github-society-intelligence/docs/network-policy.md` listing the required outbound endpoints with ports and protocols.
...
|---|---|---|
| **A: Agent pushes to a PR branch** | Modify `agent.ts` to push to `gsi/issue-<N>` and open a PR via `gh pr create` | Agent changes become reviewable; requires significant `agent.ts` refactor |
...
**File**: `.github/workflows/github-society-intelligence-agent.yml`, lines 77–82.
...
**Path**: `.github-society-intelligence/config/restricted-paths.json`
...
const restrictedPaths = JSON.parse(
  await Bun.file(".github-society-intelligence/config/restricted-paths.json").text()
...
If branch protection instructions are added to the installation section, ensure both the canonical README at `.github-society-intelligence/README.md` and the root `README.md` copy are updated.
