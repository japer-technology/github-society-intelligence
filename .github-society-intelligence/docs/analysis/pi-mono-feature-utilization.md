...
GitHub Society Intelligence depends on a single package from the [pi-mono](https://github.com/earendil-works/pi) monorepo — `@earendil-works/pi-coding-agent`. This document audits which pi-mono features GSI currently uses, which remain untapped, and which additions deliver the highest value for a "GitHub as Infrastructure" deployment where the agent runs non-interactively in `--mode json` inside GitHub Actions.
...
| Package | Description | Used by GSI |
|---|---|---|
...
GSI installs only `pi-coding-agent` and relies on it both as a CLI binary and as the provider of the `.pi/` configuration contract (settings, skills, extensions, context files).
...
Pi loads `AGENTS.md` at startup for project-specific instructions and agent identity. GSI maintains this at `.github-society-intelligence/AGENTS.md`.
...
**Impact of omission:** Long multi-turn conversations on a single issue can exhaust the model's context window. Pi's default auto-compaction is enabled, but GSI does not tune `reserveTokens` or `keepRecentTokens` for its specific usage pattern (GitHub Actions with large tool outputs).
...
**Recommendation:** Defer. The coding agent's tool-calling capability is central to GSI's value proposition. Direct API usage would add a second dependency and a second code path for marginal efficiency gains.
...
Three pi-mono packages are not recommended for GSI integration:

| Package | Reason for Exclusion |
|---|---|
| `pi-mom` | Slack bot; GSI's interface is GitHub Issues, not Slack |
| `pi-tui` | Terminal UI; GSI runs headless in GitHub Actions |
| `pi-pods` | GPU pod management; GSI uses hosted LLM APIs, not self-hosted models |

---

## 9. Summary

GSI currently uses 7 of pi-mono's feature categories (CLI invocation, session management, settings, system prompt extension, bootstrap protocol, context files, and skills). The highest-impact additions are compaction and retry settings (zero-code, configuration-only), followed by prompt templates and a lightweight GitHub context extension.
...
The upgrade to `@earendil-works/pi-coding-agent` v0.75.5 (see [pi-mono-upgrade-65.1-to-75.5.md](./pi-mono-upgrade-65.1-to-75.5.md)) unlocks several capabilities worth re-auditing for GSI utilization:

- **Incremental bash streaming** (v0.73.0) — long-running commands emit output progressively via `message_update` events; relevant for GSI's JSONL consumers.
