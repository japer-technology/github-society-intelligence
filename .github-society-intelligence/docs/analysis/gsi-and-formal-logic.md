# Formal Logic — Applied to GitHub Society Intelligence
...
This document translates the core claims of GitHub Society Intelligence (GSI) into formal logical structures, evaluating each for validity (does the conclusion follow?), soundness (are the premises true?), and completeness (are all necessary premises stated?).
...
P3: GSI's agent operates within the repository (the substrate of the work).
∴ C: GSI's agent eliminates the context gap.
...
| P3      | **True** | GSI runs in a full checkout on a GitHub Actions runner. This is verifiable. |
...
P2: GSI is built from existing GitHub components (Issues, Actions, Git).
P3: Dedicated AI infrastructure requires new components (backend, 
    auth, data store).
∴ C: GSI has lower operational cost and higher reliability than 
    dedicated AI infrastructure.
    [Modus Ponens: P1 ∧ P2 → lower cost/higher reliability for GSI;
     contrast with P3]
```

### Validity Assessment

**Valid.** If P1 is true and P2 correctly characterizes GSI, C follows.
...
| P2      | **True** | Verifiable from the codebase. GSI uses no custom backend. |
| P3      | **True** | Any equivalent dedicated system would require at minimum a compute layer and a state layer. |

### Hidden Premises

- **HP1:** GitHub's primitives are sufficiently expressive for GSI's interaction model. (If Issues cannot represent structured conversations, the cost of working around limitations may exceed the cost of building a purpose-built conversation layer.)
...
P2: Every action of GSI's agent is a Git commit — auditable, 
    reversible, and governed by repository access control.
∴ C: GSI's agent is trustworthy.
...
P2': Most GSI agent actions are Git commits; reasoning is not 
     currently captured.
∴ C': GSI's agent is more trustworthy than an unauditable equivalent, 
...
- **HP2:** Access control is correctly configured. (GSI's own security assessment identifies SEC-005: no branch protection.)
...
P2: GSI encodes its ethical constraints as versioned 
    Markdown in the repository.
P3: The repository is the substrate GSI operates on.
∴ C: GSI's ethical constraints are more likely to be maintained and 
...
**Valid.** If co-location improves maintenance and enforcement (P1), and GSI co-locates its constraints (P2, P3), then the conclusion follows.
...
P3: GSI's bootstrap protocol defines identity through human-agent 
    dialogue.
∴ C: GSI's agent outputs are more aligned with the human's needs.
...
The most common logical error across GSI's arguments is conflating necessary and sufficient conditions. Auditability is *necessary* for trust, not *sufficient*. Co-location is *necessary* for maintenance, not *sufficient* for enforcement. Dialogue is a *contributor* to alignment, not a *guarantee*.
