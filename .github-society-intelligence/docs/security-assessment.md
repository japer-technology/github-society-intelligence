...
>
> **System Under Review:** `github-society-intelligence` (GSI) agent running on `gsi-test-1`
>
...
The `github-society-intelligence` system is an AI coding agent that runs autonomously inside GitHub Actions, triggered by issue events. It can read files, execute arbitrary bash commands, edit code, and push changes to the repository.
...
**Description:** The `GITHUB_TOKEN` issued to this workflow is an organization-scoped installation token. It grants read/write access to all 24 repositories in the `japer-technology` organization, not just `gsi-test-1`.
...
3. **Scope `GITHUB_TOKEN`** - use a fine-grained PAT with:
   - Repository access: `gsi-test-1` only
...
|----------|---------|----------|--------|
| Repository access | 24 repos (org-wide) | 1 repo (`gsi-test-1`) | 23 repos excess |
...
| 2 | **Scope GITHUB_TOKEN** - replace with fine-grained PAT limited to `gsi-test-1` | Medium | Reduces scope of access to this repository only |
...
|------|---------|--------|---------|
| 2026-02-24 | 1.0 | AI Agent (GSI) | Initial security report |
