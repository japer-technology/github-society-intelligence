...
If *[The Repo Is the Mind](the-repo-is-the-mind.md)* is the thesis, then this repository is its field evidence. The “where” is not metaphorical. Society Intelligence answers it with a physical address: inside the repository’s own boundaries, executed by the same automation that already builds and ships code.
...
The workflow (`.github/workflows/github-society-intelligence-agent.yml`) is explicit: issue events and issue comments trigger a job, collaborator permissions are checked, the repository is checked out, Bun is installed, and the agent scripts run locally in the Actions runner.
...
The orchestrator (`.github-society-intelligence/lifecycle/agent.ts`) persists issue-to-session mappings and session transcripts under `.github-society-intelligence/state/`. The README repeats the same mapping model: issue number → state file → session transcript. This is not decorative documentation; it is the mechanism of continuity.

So “where memory lives” is no longer a hand-wavy concept about context windows. It lives in:

- `.github-society-intelligence/state/issues/<number>.json`
- `.github-society-intelligence/state/sessions/<session>.jsonl`
...
This is subtle but essential. By making installation file-centric, the project ensures the answer to “where is the product?” remains simple: in `.github-society-intelligence/` plus generated `.github/` workflow/template files. The “platform” is a commit.
