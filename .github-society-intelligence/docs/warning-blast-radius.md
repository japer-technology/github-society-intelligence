...
> available to the `github-society-intelligence` (GSI) agent running as a GitHub Actions
...
- ✅ **Clone any org repository** (verified: `github-society-intelligence`, `aws-lambda-nodejs-catch-all`)
...
japer-technology/github-society-intelligence
japer-technology/gsi-test-1
... (24 total)
```
...
1. Push a modified workflow to `gsi-test-1` that removes the authorization check
...
The GSI agent, out-of-the-box, operates with **broad privilege** relative to its intended purpose (responding to GitHub issues). This is not unique to GSI — it is a property of any GitHub Actions workflow with write permissions on an organization-scoped token. The combination of:
