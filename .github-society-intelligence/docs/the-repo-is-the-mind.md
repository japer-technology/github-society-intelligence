...
The `pi` coding agent and the Society Intelligence framework eliminate that context gap by embedding the AI directly in the repository - not as a service integration, but as a versioned dependency that operates inside your existing Git, Actions, and Issues infrastructure.
...
`@earendil-works/pi-coding-agent` is an npm package. Society Intelligence is a repo-local configuration layer. You install it, it runs in your tree, and that's the entire vendor relationship.
...
On initialization, you "hatch" the agent by opening an issue and defining its persona through dialogue. The result is a Markdown file committed to the repository - typically in `.github-society-intelligence/` - that governs the agent's tone, priorities, and behavioral constraints.
