 ...
 *      `.github-society-intelligence/.github-society-intelligence/state/`.
...
describe("local-chat regression tests", () => {
  test("§3.4: --list from a foreign cwd does not nest state dirs", () => {
    const foreignCwd = mkdtempSync(join(tmpdir(), "gsi-cwd-"));
    ...
    const nested = join(MI_DIR, ".github-society-intelligence");
    ...
    expect(existsSync(join(foreignCwd, ".github-society-intelligence"))).toBe(false);
  });
