...
**Society Intelligence flips that model.** Every prompt you write and every response the agent produces is committed directly to your repository as part of its normal workflow. There is nothing to copy, nothing to paste, and nothing stored outside your control.
...
## Why GitHub Society Intelligence
...
.github-society-intelligence/
  ...
  install/
    SOCIETY-INTELLIGENCE-AGENTS.md               # Default agent identity template (copied to AGENTS.md on install)
...
**Change the model** - edit `.github-society-intelligence/.pi/settings.json`:
...
**Filter by label** - edit `.github/workflows/github-society-intelligence-agent.yml` to only trigger on issues with a specific label.
...
`.pi` supports a wide range of LLM providers out of the box. Set `defaultProvider` and `defaultModel` in `.github-society-intelligence/.pi/settings.json` and add the matching API key to your workflow:
...
If you plan to use society-intelligence for anything private, **make the repo private**. Public repos mean your conversation history is visible to everyone, but get generous GitHub Actions usage.

---

## Repo Size

The repo is overwhelmingly dominated by node_modules (~99%). The actual project files (README, LICENSE, config, GitHub workflows, GSI state/lifecycle) are only about ~1 MB.
